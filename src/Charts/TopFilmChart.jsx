...
const TopFilmChart = () => {
  ...
  const [topCount, setTopCount] = useState(5); // Hiển thị top 5 mặc định
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const mappedData = response.data.results.map(item => ({
          film_id: item.film_id || 0,
          film_title: item.film_title || item.cinema_name, // Map cinema_name to film_title if needed
        ...
        let filteredData = mappedData.filter(item => item.year === year);
        ...
        // Tính tổng doanh thu cho mỗi phim (nếu xem tất cả các tháng)
        let processedData = filteredData;
        if (month === 0) {
          const films = {};
          filteredData.forEach(item => {
            const key = item.film_id || item.film_title;
            if (!films[key]) {
              films[key] = {
            ...
            films[key].total_revenue += item.total_revenue;
          });
          processedData = Object.values(films);
        }
        ...
        if (sortedData.length === 0) {
          setError(`Không có dữ liệu doanh thu phim cho ${monthNames[month]} năm ${year}`);
          setLoading(false);
          return;
        }
        
        // Rút gọn tên phim nếu quá dài
        const shortenedLabels = sortedData.map(item => {
          const name = item.film_title;
        ...
        const titleText = month === 0 
          ? `Top ${topCount} Phim Có Doanh Thu Cao Nhất Năm ${year}` 
          : `Top ${topCount} Phim Có Doanh Thu Cao Nhất ${monthNames[month]} Năm ${year}`;
        ...
        console.error("Error fetching top film revenue data:", err);
    ...
    fetchData();
  }, [year, month, topCount, currentYear, currentMonth]);
