...
const ThemSuatChieu = ({ isModalVisible, handleCancel }) => {
  ...
  useEffect(() => {
    axios.get('/films_with_full_info.json').then((response) => setFilms(response.data));
    axios.get('/cinemas_with_full_info.json').then((response) => setCinemas(response.data));
  }, []);

  const handleCinemaChange = (cinemaId) => {
    setSelectedCinema(cinemaId);
    axios.get('/rooms_with_full_info.json').then((response) => {
      const cinemaRooms = response.data[`cinema_${cinemaId}`] || [];
  ...
  const onOk = () => {
    form.validateFields()
      .then((values) => {
        ...
        axios.post('http://localhost:5001/api/addShowtime', showtimeData)
          .then(() => {
            form.resetFields();
            handleCancel();
          })
